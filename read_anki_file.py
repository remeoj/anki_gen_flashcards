import re
import anki_export
import json 

CARD_LIST = []
APKG = anki_export.ApkgReader('anki_deck.apkg')

# Used to experiment with APKG API.
def create_clean_card(flashcard):
    card_front = list(flashcard['data']['note']['data']['record'].values())[0]
    card_front = re.sub(r'<span[^>]*>', '', card_front)
    card_front = re.sub(r'</span>', '', card_front) 
    return card_front

# Used to experiment with APKG API.
def get_flashcards_from_apkg():
    for card in APKG.cards:
        card_front = create_clean_card(card)
        CARD_LIST.append(card_front)

def check_card_in_deck(word: str) :
    for card in APKG.cards:
        if word in card:
            print(f"'{word}' was found. Please remove from the list of words to add.")
            return True
    return False

def create_new_words_list():
    new_words= []
    duplicate_words = set()
    with open('new_words.txt', 'r', encoding='utf-8') as file:
        for line in file:
            # Remove any leading/trailing whitespace and newline characters. Also removes any translation or notes I may have written.
            new_word = line.split('-')[0].strip().capitalize()
            # Ensure that duplicate entries even in the new words list aren't added.
            if not new_word in duplicate_words:
                new_words.append(new_word)
            duplicate_words.add(new_word)
    return new_words

def check_anki_for_duplicate_cards():
        new_words_list = create_new_words_list()
        found_duplicate = False
        for word in new_words_list: 
           card_exists = check_card_in_deck(word)
           if not found_duplicate and card_exists:
               found_duplicate = card_exists
        if not found_duplicate:
            print("No duplicates were found. Feel free to create new flashcards!")
            return True
        else:
            print("Duplicates were found, please advise, then restart. Stopping program early.")
            return False

# Uncomment to run card test
# check_anki_for_duplicate_cards()